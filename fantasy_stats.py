...
class UpdateData():
    ...
    def UpdateTransactions(self):
        ...
        url = 'https://fantasysports.yahooapis.com/fantasy/v2/league/380.l.180335/transactions'
    ...
    def UpdateLeague(self):
        # LEAGUE OVERVIEW
        yahoo_api._login()
        url = 'https://fantasysports.yahooapis.com/fantasy/v2/league/380.l.180335/'
        response = oauth.session.get(url, params={'format': 'json'})
        r = response.json()
    ...
    def UpdateLeagueStandings(self):
        # STANDINGS
        yahoo_api._login()
        url = 'https://fantasysports.yahooapis.com/fantasy/v2/league/380.l.180335/standings'
        response = oauth.session.get(url, params={'format': 'json'})
        r = response.json()
    ...
    def UpdateScoreboards(self):
        ...
        while week < num_weeks+1: #assumes 16 week-schedule
            url = 'https://fantasysports.yahooapis.com/fantasy/v2/league/380.l.180335/scoreboard;week='+str(week)
            response = oauth.session.get(url, params={'format': 'json'})
            r = response.json()
    ...
    def UpdateRosters(self):
        ...
        for week in range(1, num_weeks+1): #assumes 16-week schedule
            ...
            for team in range(1, num_teams+1): #assumes 12-team league
                url = 'https://fantasysports.yahooapis.com/fantasy/v2/team/380.l.180335.t.'+str(team)+'/roster;week='+str(week)
                response = oauth.session.get(url, params={'format': 'json'})
                r = response.json()
